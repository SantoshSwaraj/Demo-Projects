...
public class AddToLogFlowMonitorExcel implements Excel{
	...
	public AddToLogFlowMonitorExcel() {
		rowCount = 1;
		workbook = new XSSFWorkbook();
	}
	
	public AddToLogFlowMonitorExcel(List<Object> liObjects) {
		rowCount = 1;
		workbook = new XSSFWorkbook();
	...
	public void writeHeader() {
		...
		AppConstants.LOG_FLOW_DETECTED_DEVICES_EXPORT_COLUMN.stream().forEach(e -> createCell(row, columnCount.getAndIncrement(), e, style));
	...
	public void write() {
		...
		listRecords.forEach(obj -> {
			...
			AddToLFMResponse lfmResponse = (AddToLFMResponse) obj;
			...
			createCell(row, columnCount.getAndIncrement(), getLocalDateTimeFromMilliSec(lfmResponse.getDetectedDate()), style);
			createCell(row, columnCount.getAndIncrement(), getLocalDateTimeFromMilliSec(lfmResponse.getLastEventReceived()), style);
